...
func main() {
	flag.Parse()
	if flag.NArg() != 1 {
		log.Fatalf("Usage: %s pkg/cloudprovider/zz_generated.spot_pricing.go", os.Args[0])
	}
	...
	for {
		if pricingProvider.SpotLastUpdated().After(updateStarted) {
			break
		}
	...
	fmt.Fprintf(src, "var initialSpotPriceUpdate, _ = time.Parse(time.RFC3339, \"%s\")\n", now)

	instanceTypes := pricingProvider.InstanceTypes()
	sort.Strings(instanceTypes)

	writePricing(src, instanceTypes, "initialSpotPrices", pricingProvider.SpotPrice)
...
func writePricing(src *bytes.Buffer, instanceNames []string, varName string, getPrice func(instanceType string, zone string) (float64, bool)) {
	...
	for _, instanceName := range instanceNames {
		...
		price, ok := getPrice(instanceName, "us-east-1a")
		if !ok {
			continue
		}
